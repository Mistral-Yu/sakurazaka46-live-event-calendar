...
- 表示モードは `live` / `event` / `all` / `next14`、タブ表示名は `LIVE` / `EVENT` / `ALL` / `直近2週間`。デフォルト表示は `live`。
- URL状態は `?mode=live` / `?mode=event` / `?mode=all` / `?mode=next14` を使う。
...
- `all` の日付セルチップは詳細分類する:
  - `ライブ開催` と `イベント開催` は別チップ。ただし同じピンクの開催色。
  - `ライブ抽選` と `イベント応募` は別チップ。ただし同じ青の応募・抽選色。
  - `ライブ抽選締切` と `イベント応募締切` は別チップ。ただし同じ赤の締切色。
- `all` チップの重複は同一ラベル内だけ畳む。例: 複数の live 抽選は `ライブ抽選` 1つに畳むが、`ライブ抽選` と `イベント応募` は両方表示する。
